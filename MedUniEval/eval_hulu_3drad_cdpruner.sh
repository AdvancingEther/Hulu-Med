...
MODEL_NAME="Hulumed_qwen2_cdpruner"
...
REASONING="False"
TEST_TIMES="1"
CDPRUNER_ENABLE="true"
CDPRUNER_ALPHA="${CDPRUNER_ALPHA:-1.0}"
...
CDPRUNER_TAGS=("50" "70")
CDPRUNER_KEPT_RATIOS=("0.50" "0.70")

run_experiment() {
    local rad3d_num_slices="$1"
    local cdp_tag="$2"
    local cdpruner_kept_ratio="$3"
    local -a fixed_resolution_args=()

    local output_tag="cdp_${cdp_tag}"
    ...
    echo "========================================"
    echo "Running slice=${rad3d_num_slices}, cdpruner=${cdp_tag} (kept_ratio=${cdpruner_kept_ratio}, alpha=${CDPRUNER_ALPHA})"
    ...
    if [ "$CHUNKS" -eq 1 ]; then
        (
            ...
            "$PYTHON_BIN" scripts/infer_only.py \
                ...
                --cdpruner_enable "$CDPRUNER_ENABLE" \
                --cdpruner_kept_ratio "$cdpruner_kept_ratio" \
                --cdpruner_alpha "$CDPRUNER_ALPHA" \
                --use_vllm "$USE_VLLM" \
                --reasoning "$REASONING" \
...
for rad3d_num_slices in "${SLICE_SETTINGS[@]}"; do
    for idx in "${!CDPRUNER_TAGS[@]}"; do
        run_experiment \
            "$rad3d_num_slices" \
            "${CDPRUNER_TAGS[$idx]}" \
            "${CDPRUNER_KEPT_RATIOS[$idx]}"
    done
done
